...
def set_ped(targetIds, pos_x, pos_y):
  ped = {
        "source" : None,
        "targetIds" : [ targetIds ],
        "position" : {
...
ped = set_ped(2, 11.6, 1.5)
tmp.append(ped)
data["scenario"]["topography"]["dynamicElements"] = tmp

#save the file with new name
out_file = open(current_path + new_filename, "w+")
out_file.write(json.dumps(data))
out_file.close()

#Call vadere-console.jar
os.system('cmd /c "java -jar vadere-console.jar scenario-run --scenario-file rimea_06_corner_task3.scenario --output-dir output')